...
flowchart LR
    ...
    Bus -->|invoice.submitted| Decision["Decision Engine<br/>(AI classify → PolicyEngine)"]
    Decision -->|AI classify| AI["IAiModelProvider<br/>Groq / Stub"]
...
sequenceDiagram
    ...
    alt already submitted
        ...
        DE->>AI: AnalyzeAsync(invoice)
        AI-->>DE: AiAnalysisResult {category, confidence, metadata}
        DE->>PE: EvaluateAsync(invoice, aiResult)
...
is classification and extraction only; it must not be able to talk its way past a ceiling.

### Decision
- **PolicyEngine is deterministic and category-agnostic to the AI's opinion of itself.**
...
- **`IAiModelProvider` is an anti-corruption layer (M15).** `StubAiModelProvider` is a
  ...
  and is selected via the `AiProvider` config key — swapping providers is a config
...
### Consequences
- **Positive:** M12's proof does not depend on trusting the AI's category choice — the
